...
namespace S100Framework.Applications
{
    ...
    public class DynamicFileSink : ILogEventSink
    {
        ...
        public void Emit(LogEvent logEvent) {
            ...
            var filePath = Path.Combine(this._logDirectory, $"ImporterYAML_DataObject_{logFileName}.log");
    ...
    internal static class Logger
    {
        ...
        internal static string LogDir { get => System.IO.Path.Combine(_logDir, @"S-100 BlueStack", "ImporterYAML", $"{_dateTimeString}"); }
        ...
        static Logger() {
            _logger = new LoggerConfiguration()
                ...
                .WriteTo.Logger(lc => lc
                    ...
                    .WriteTo.File(System.IO.Path.Combine(_logDir, @"S-100 BlueStack", "ImporterYAML", $"{_dateTimeString}", "ImporterYAML_System.log"),
                ...
                .WriteTo.Logger(lc => lc
                    .Filter.ByIncludingOnly(e => e.Level == (LogEventLevel)6)
                    .WriteTo.Sink(new DynamicFileSink(System.IO.Path.Combine(_logDir, @"S-100 BlueStack", "ImporterYAML", $"{_dateTimeString}"))))

                .WriteTo.Logger(lc => lc
                    .WriteTo.Console()
                    .Filter.ByIncludingOnly(e => e.Level == (LogEventLevel)7)
                    .WriteTo.File(System.IO.Path.Combine(_logDir, @"S-100 BlueStack", "ImporterYAML", $"{_dateTimeString}", $"ImporterYAML_DataTotalCount.log"),
                    rollingInterval: RollingInterval.Infinite,
                    shared: true,
                    encoding: System.Text.Encoding.GetEncoding("ISO-8859-1"),
                ...
                .WriteTo.Logger(lc => lc
                    ...
                    .WriteTo.File(System.IO.Path.Combine(_logDir, @"S-100 BlueStack", "ImporterYAML", $"{_dateTimeString}", "ImporterYAML_DataError.log"),
                    rollingInterval: RollingInterval.Infinite,
                    shared: true,
                    encoding: System.Text.Encoding.GetEncoding("ISO-8859-1"),
                    outputTemplate: "{Message:lj}{NewLine}"))
