...
namespace S100Framework.WPF.ViewModel
{
    ...
    public class attributeBindingDefinitionViewModel : attributeBindingDefinition
    {
        public Action<AddError, attributeBinding>[] Validators { get; set; } = [];
    }
    ...
    public class SimpleAttributeViewModel : AttributeViewModel, INotifyDataErrorInfo
    {
        ...
        public object? value {
            ...
            set {
                this.attribute.GetType().GetProperty("value")!.SetValue(this._attribute, value);
        ...
        private attributeBindingDefinition? _attributeBindingDefinition { get; init; } = default;

        private Action<AddError, attributeBinding>[]? _validators { get; init; } = default;
        ...
        public DateTime? value {
            ...
            set {
                if (value != null) {
                    ...
                    this.attribute.GetType().GetProperty("value")!.SetValue(this._attribute, date);
                }
                else
                    this.attribute.GetType().GetProperty("value")!.SetValue(this._attribute, default);
                this.SetProperty(ref this._value, value);
            }
        }
    ...
    public class InformationBindingViewModel : INotifyPropertyChanged
    {
        ...
        public IEnumerable GetErrors(string propertyName) => "associations".Equals(propertyName) ? this._errors.Values.SelectMany(x => x) ?? Enumerable.Empty<string>() : Enumerable.Empty<string>();

        private void ValidateProperty(string propertyName, object? value) {
            if (propertyName == nameof(this.roleType)) {
                var s = (string?)value ?? string.Empty;
                this.SetError(propertyName, string.IsNullOrWhiteSpace(s) ? "roleType is required." : null);
            }
            if (propertyName == nameof(this.role)) {
                var s = (string?)value ?? string.Empty;
                this.SetError(propertyName, string.IsNullOrWhiteSpace(s) ? "role is required." : null);
            }
            if (propertyName == nameof(this.informationType)) {
                var s = (string?)value ?? string.Empty;
                this.SetError(propertyName, string.IsNullOrWhiteSpace(s) ? "informationType is required." : null);
            }
            if (propertyName == nameof(this.informationUID)) {
                this.SetError(propertyName, value is null ? "informationUID is required." : null);
            }
        ...
        private void SetError(string propertyName, string? error) {
            var had = this._errors.Remove(propertyName);
        ...
        public InformationBindingViewModel(IGrouping<string, informationBindingDefinition> informationBinding) {
            ...
            this.PropertyChanged += (s, e) => {
                ...
                else if (e.PropertyName.Equals(nameof(this.role))) {
                ...
                else if (e.PropertyName.Equals(nameof(this.informationType))) {
                }
                else if (e.PropertyName.Equals(nameof(this.informationUID))) {
            ...
            this.ValidateProperty(nameof(this.roleType), this.roleType);
            this.ValidateProperty(nameof(this.role), this.role);
            this.ValidateProperty(nameof(this.informationType), this.informationType);
            this.ValidateProperty(nameof(this.informationUID), this.informationUID);
        }
        ...
        private readonly informationBindingDefinition[] _informationBindingDefinitions;
    }
    ...
    public class FeatureBindingViewModel : INotifyPropertyChanged, INotifyDataErrorInfo
    {
        ...
        private void ValidateProperty(string propertyName, object? value) {
            ...
            if (propertyName == nameof(this.featureType)) {
                var s = (string?)value ?? string.Empty;
                this.SetError(propertyName, string.IsNullOrWhiteSpace(s) ? "featureType is required." : null);
            }
            if (propertyName == nameof(this.featureUID)) {
                this.SetError(propertyName, value is null ? "featureUID is required." : null);
            }
        ...
        public FeatureBindingViewModel(IGrouping<string, featureBindingDefinition> featureBindings) {
            ...
            this.PropertyChanged += (s, e) => {
                ...
                else if (e.PropertyName.Equals(nameof(this.featureType))) {
                }
                else if (e.PropertyName.Equals(nameof(this.featureUID))) {
            ...
            this.ValidateProperty(nameof(this.roleType), this.roleType);
            this.ValidateProperty(nameof(this.role), this.role);
            this.ValidateProperty(nameof(this.featureType), this.featureType);
            this.ValidateProperty(nameof(this.featureUID), this.featureUID);
        }
        ...
        private readonly featureBindingDefinition[] _featureBindingDefinitions;
