...
internal static class Program
{
    private const string Usage = """
        s100json - convert an ISO/IEC 8211 (S-100 / S-101 / S-57) file to JSON

          s100json <input> [options]

        Options
          -o, --out <path>        Output file. Defaults to <input>.json, or stdout with "-".
    ...
    private static void Convert(ConversionOptions options) {
        ...
        if (options.Mode == ConversionMode.Raw) {
            Iso8211JsonWriter.Write(reader, target, options.Indented, Path.GetFileName(options.InputPath));
            return;
        }

        var dataset = S101DatasetReader.Read(reader, options.S101Options, Path.GetFileName(options.InputPath));
        S101JsonWriter.Write(dataset, target, options.Indented, geoJsonOnly: options.Mode == ConversionMode.GeoJson);
...
internal sealed class ConversionOptions
{
    ...
    /// <summary>The output path the user asked for, or "&lt;input&gt;.json" when they did not say.</summary>
    public string ResolvedOutputPath => OutputPath ?? InputPath + ".json";
